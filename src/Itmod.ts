...
import {BigMap, BigSet} from "./collections/big";
...
>(
    ...
    return {
        *[Symbol.iterator]() {
            const rightIndex = new BigMap<K, R[]>();
            ...
            for (const leftValue of left) {
                ...
                // if (!firstLeft) break;
                // firstLeft = false;
                let rightNext = rightIter.next();
                while (!rightNext.done) {
                    const rightValue = rightNext.value;
            ...
            if (
                ...
                for (const rightKey of unmatchedRight) {
                    for (const rightValue of rightIndex.get(rightKey)!) {
                        yield (joiner as any)(undefined, rightValue);
                    }
                }
            }
        },
...
console.log("average:");
console.log(times.reduce((t, c) => t + c) / times.length);
